...
> **Vendored from [statzhero/tidy-r-skill](https://github.com/statzhero/tidy-r-skill)** by
> **Ulrich Atz** (CC-BY-4.0; repo also carries an MIT `LICENSE`, included here).
> Bundled into pi-quant-toolkit with attribution; see `LICENSE` and
> `UPSTREAM-README.md` in this folder. Targets R >= 4.5, tidyverse >= 2.0,
> dplyr >= 1.2 — a few functions it recommends are recent; if one isn't in your
> installed dplyr, use the `case_when()` / `na.rm = TRUE` equivalents it lists.
